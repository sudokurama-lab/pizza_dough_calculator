    ...
    <div class="space-y-4">
      ...
      <div class="bg-orange-50 p-1 rounded-lg flex space-x-1 mb-6">
        <button 
          ...
          :class="calcMode === 'target_weight' ? 'bg-white shadow-sm text-orange-700' : 'text-gray-500 hover:text-gray-700'"
        ...
        <button 
          ...
          :class="calcMode === 'total_flour' ? 'bg-white shadow-sm text-orange-700' : 'text-gray-500 hover:text-gray-700'"
          class="flex-1 py-2 text-sm font-medium rounded-md transition-all"
        >
          Por Harina Total
        </button>
      ...
      <template v-if="calcMode === 'target_weight'">
        <div>
          ...
          <input type="number" v-model="pizzas" min="1" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 p-2.5 border bg-gray-50" />
        </div>
      </template>

      <template v-else>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Harina disponible (gramos)</label>
          <input type="number" v-model="availableFlour" min="100" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 p-2.5 border bg-gray-50" />
        </div>
      </template>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Peso por pizza (gramos)</label>
        <input type="number" v-model="weightPerPizza" min="100" step="10" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 p-2.5 border bg-gray-50" />
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Hidratación (%)</label>
        <input type="number" v-model="hydration" min="50" max="100" step="1" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 p-2.5 border bg-gray-50" />
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Sal (%)</label>
        <input type="number" v-model="salt" min="1" max="5" step="0.1" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 p-2.5 border bg-gray-50" />
      </div>
